...
Categories: routing (plain HR question), ambiguous_jurisdiction (guardrail
      should trigger), action (draft_pto_request), escalation (no-match ->
      create_hr_ticket), permission (RBAC-gated compensation topic).
"""
...
def build_eval_set() -> EvalSet:
    ...
    return EvalSet(
        ...
        description=(
            "Covers routing, jurisdiction-ambiguity guardrail, action tools "
            "(draft_pto_request), HITL escalation (create_hr_ticket) and "
